...
async function prepareOrderMessage(form) {
  ...
  const raribleEncodedOrderURL = `${apiDomain}/protocol/v0.1/ethereum/encoder/order`;
  const res = await axios.post(raribleEncodedOrderURL, JSON.stringify(form), {
...
function createTypeData(domianData, primaryType, message, types) {
  return {
    types: {
      EIP721Domain: DOMAIN_TYPE,
      ...types,
    },
    domain: domianData,
    primaryType,
