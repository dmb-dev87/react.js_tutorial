...
import { createSellOrder } from "../../utils/sellOrderRequest";
...
export default function Details() {
  ...
  const fetchTokenData = async () => {
    let { data } = await axios.get(
      currentChainInfo.apiDomain + `/protocol/v0.1/ethereum/nft/items/${tokenId}`,
  ...
  const fetchTokenSellOrder = async () => {
    ...
    let { data } = await axios.get(
      currentChainInfo.apiDomain + "/protocol/v0.1/ethereum/dorder/sell/byItem",
      { params: { contract, tokenId: token }}
    );

    let sortedData = data.order.sort(
      //@ts-ignore
